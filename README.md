...
```
GitHub Actions (cron) ──trigger──▶ Bright Data Scraper Studio (Sitemap collectors c_* x7)
        ▲                                     │
...
2. **Collectors** — one per source (see `collectors/collectors.json` for prompts and IDs):
   ...
   bash scripts/setup_collectors.sh --execute  # creates all collectors
   ```
3. **Environment** — copy `.env.example` to `.env`, fill in `BRIGHT_DATA_API_TOKEN`, `BRIGHT_DATA_COLLECTOR_IDS`, `OPENROUTER_API_KEY`.
4. **Install & run**:
   ```bash
...
- **7 custom Sitemap scrapers** (Docker, Kubernetes, AWS EKS, Argo CD, GitHub Actions, LangChain, + a fixture demo site) created via the Bright Data CLI / AI Agent — none of these targets exist in the pre-built scrapers library.
...
- `scripts/` — `run_refresh.sh`, `setup_collectors.sh`, `break_fixture.sh`, `restore_fixture.sh`
...
- `tests/` — unit + integration tests (chunking, diff, health rules, mocked Bright Data client, schemas, index, heal, RAG, API endpoints, CLI, output consistency)
